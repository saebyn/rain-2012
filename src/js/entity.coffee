require.ensure ['lib/gamejs'], (require) ->
  gamejs = require 'lib/gamejs'
  $v = gamejs.utils.vectors

  class Character extends gamejs.sprite.Sprite
    # TODO geometry related test asserts
    # TODO use test asserts to TDD the handleCollision method
    # TODO scene object that has viewport to world coords mapping + groups (like solid_entities)
    constructor: (@solid_entities) ->
      @direction = [0.0, 0.0]
      super()

    handleCollision: (oldCenter, movement) ->
      # TODO changes:
